// Importamos los hooks necesarios y las funciones para interactuar con la API de cartas.
...
const Juego = () => {
  ...
  const tomarCarta = async () => {
    try {
      ...
      if (nuevaPuntuacionRonda > 21) {
        setmensaje("Su puntuacion es mayor a 21, has perdidoooo")
        botonPlantar(false)
        setPuntuacionJugador((prevPuntuacion) => prevPuntuacion + valorCarta);
  ...
  // Función para finalizar la ronda, actualizando la puntuación total del jugador y reiniciando la ronda actual.
  const plantar = async () => {
  ...
  const reiniciarJuego = async () => {
    try {
      ...
      setCartasPc([]);
      setmensaje("");
      setEmpezarJuego(true); // Habilitar el botón "Empezar" nuevamente
      setPlantarJugador(false); // Restablecer el estado del jugador (no plantado)
      setbotonPlantar(false); // Habilitar el botón "Plantar"
    } catch (error) {
      console.error(error.message);
    }
  };
